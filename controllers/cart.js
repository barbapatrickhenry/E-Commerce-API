  ...
  try {
    ...
    if (userCart.length > 0) {
      ...
      return res.status(404).send({
        message: "The User's Cart Does Not Exist at the Moment.",
      });
    }
  } catch (error) {
    console.error("Error in Finding the Cart", error);
    return res.status(500).send({ error: "Error Finding the Cart of the User" });
  }
};
...
module.exports.removeFromCart = async (req, res) => {
  try {
    ...
    cart.cartItems = cart.cartItems.filter(item => !productId.includes(item.productId));
